...
void MinasClient::reset()
{
    ...
    output.controlword = 0x00;  // fault reset
    writeOutputs(output);
    ...
    uint8_t alStatus = readALStatus();
    resetErrorCode();
    printf("Read status is [%d]" , alStatus);
    printf("RESET 2");
    printf("Slave number is [%d]",slave_no_);
    int loop = 0;
    while (input.error_code != 0 && ros::ok())
    {
        if (loop++ % 100 == 1)
        {
            printf("Input is!");
            ...
            printf("Output is!");
            printf("controlword = %04x, operation_mode %04x, target_torque = %d, max_torque = %d, target_position = %d, max_motor_speed = %d",
                   output.controlword, output.operation_mode, output.target_torque, output.max_torque, output.target_position, output.max_motor_speed);
...
void MinasClient::resetErrorCode(void)
{
    manager_.writeSDO<uint16_t>(slave_no_, 0x0120, 0x00, 0x0010);
}

uint8_t MinasClient::readALStatus(void)
{
    return manager_.readSDO<uint8_t>(slave_no_,0x0135,0x00);
}
...
int32_t MinasClient::read_actual_pos(void)
{
    // 6063h
    return manager_.readSDO<uint32_t>(slave_no_,0x6063,0x00);
}

int32_t MinasClient::read_homeset(void)
{
    // 607Ch
    return manager_.readSDO<uint32_t>(slave_no_,0x607C,0x00);
}

void MinasClient::set_home_pos(int32_t val)
{
    // 607Ch
    int32_t u32val = (int32_t)val;
    manager_.writeSDO<int32_t>(slave_no_, 0x607C, 0x00, u32val);
}

uint16_t MinasClient::read_inhibit_setup(void)
{
    // 3504h
    return manager_.readSDO<uint16_t>(slave_no_,0x3504,0x00);
}

void MinasClient::set_inhibit_setup(uint16_t val)
{
    // 3504h
    uint16_t u16val = (uint16_t)val;
    manager_.writeSDO<uint16_t>(slave_no_, 0x3504, 0x00, u16val);
}
