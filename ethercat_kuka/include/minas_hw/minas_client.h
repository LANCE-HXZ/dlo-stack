...
public:
   ...
   */
    ...
    uint8_t readALStatus(void);

    /* below fucs added in 2020.07.13 by cdy */
    /**
     * \brief return current actual position 6063h
    */
    int32_t read_actual_pos(void);
    
    /**
     * \brief return current homeset 607Ch
    */
    int32_t read_homeset(void);

    /**
     * \brief setting current position to home position 607Ch
    */
    void set_home_pos(int32_t val);

    /**
     * \brief return inhibit setup, in here we want it is 2. Just set error when triger POT or NOT. 3504h
    */
    uint16_t read_inhibit_setup(void);

    /**
     * \brief set inhibit setup.
    */
    void set_inhibit_setup(uint16_t val);
