...
#define L_DZ -0.005        //  左夹指夹取位置相对于原配夹指的位移(比原配夹指长为正, 数字增加表示目标点更高)
// #define L_DZ 0.03        //  左夹指夹取位置相对于原配夹指的位移(比原配夹指长为正, 数字增加表示目标点更高)
#define R_DZ 0         //  右夹指夹取位置相对于原配夹指的位移(比原配夹指长为正, 数字增加表示目标点更高)
...
class CIiwaServo{
    private:
        ...
        void SetLeftMoveMode(int nMoveMode=0, int nPathMode=0, double dMoveDuration=10.01);
        void SetRightMoveMode(int nMoveMode=0, int nPathMode=0, double dMoveDuration=10.01);
    ...
    public:
        ...
        /*  返回Lefthome位置    */
        void MoveLeftToHome(double dMoveDuration=10.01);
        /*  以相机坐标系为准，移动机械臂以指定位姿移动到指定点       alpha为沿X轴旋转到的目标角度    beta为沿Y轴旋转到的目标角度   gamma为沿Z轴旋转到的目标角度*/
        double MoveLeftEulerXYZ(double dX=0.6, double dY=0.225, double dZ=0.669, double dOx=0, double dOy=0, double dOz=-90, 
                              double dMoveDuration=10.01, int nPathMode=0);
            double MoveLeftEulerXYZ(Point ptTarget,double dZ=0.94,double dOz=0,double dMoveDuration=10.01,double dOx=0,double dOy=0,int nPathMode=0);
        ...
        /*返回Righthome位置*/
        void MoveRightToHome(double dMoveDuration=10.01);
        /*以相机坐标系为准，移动机械臂以指定位姿移动到指定点       alpha为沿X轴旋转到的目标角度    beta为沿Y轴旋转到的目标角度   gamma为沿Z轴旋转到的目标角度*/
        double MoveRightEulerXYZ(double dX=-0.6,double dY=0.225,double dZ=0.7,double dOx=0,double dOy=0,double dOz=-90,double dMoveDuration=10.01,int nPathMode=0);
        /*以相机坐标系为准，移动机械臂以指定位姿移动到指定点       x,y,z,w为目标点四元数*/
            double MoveRightEulerXYZ(Point ptTarget,double dZ=0.94,double dOz=0,double dMoveDuration=10.01,double dOx=0,double dOy=0,int nPathMode=0);
        ...
        /*  计算等速运动所需的时间, 根据当前位置xyz和目标xyz计算距离后除以速度得到所需运动周期
            ...
            输出: 所需运动周期(s) */
        double UniformTime(char cSide, vector<double> vdTargetPos, double dSpeed = 0.05);
        ...
        void DloMoveEulerIncrease(vector<double> dDLeft, vector<double> dDRight, double dMoveDuration=10.01, int nPathMode=0);
