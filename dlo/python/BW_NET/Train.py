...
def train(epoch_num=100, lr=LR, show_vgg_params=False, ):
    ...
    for epoch in range(epoch_num):  # 开始迭代
        ...
        prin    t('epoch train loss = %f, epoch test loss = %f, %s'
              % (train_loss / len(train_dataloader), test_loss / len(test_dataloader), time_str))

        if np.mod(epoch+1, 5) == 0:
            torch.save(fcn_model, '/home/lance/Workspaces/hxz_ws/src/dlo/python/BW_NET/Model/fcn_model_{}.pt'.format(epoch+1))
