...
void visualization(vector<int64> classlist,vector<int64> crosslist)
{
	...
	for(int n = 0; n < point_num; ++n){
		...
		if(n == cpt[cpt_index]){
			cout << '\t' << line_index  << '\t' << cpt_index << '\t' << crosslist[cpt_index] << '\t' << classlist[cpt_index] << endl;
			if(1 == classlist[cpt_index])
			{
				// // 条线连接交叉处
				// line(visual_img, pt[cpt[cpt_index]], pt[cpt[cpt_index]-1], color_value, color_size);
				// // 两条线连接交叉处
				line(visual_img, pt[cpt[cpt_index]], cross[crosslist[cpt_index]], color_value, color_size);
				line(visual_img, cross[crosslist[cpt_index]], pt[cpt[cpt_index]-1], color_value, color_size);
