...
robot_model_loader("robot_description")                 //                      */
{
    ...
    SetFrame(reference_frame);
    // L.setMaxVelocityScalingFactor(MAX_V);        //  /*
    // L.setMaxAccelerationScalingFactor(MAX_A);
    // R.setMaxVelocityScalingFactor(MAX_V);        //      设置最大速度
    // R.setMaxAccelerationScalingFactor(MAX_A);    //      设置最大加速度
    // D.setMaxVelocityScalingFactor(MAX_V);
    // D.setMaxAccelerationScalingFactor(MAX_A);    //                      */
    FOR(i, 0, 7){
...
void KukaMoveit::SetPose(geometry_msgs::Pose& target_pose, double x, double y, double z, vector<double> o){
...
void KukaMoveit::Setdxdydz(geometry_msgs::Pose& target_pose, string EE, double dx, double dy, double dz){
...
void KukaMoveit::SetJoint(vector<double>& target_joint, double a6, double a5, double a4, double a3, double a2, double a1, double a0){
    target_joint[0] = a0;
    target_joint[1] = a1;
    target_joint[2] = a2;
    target_joint[3] = a3;
...
void KukaMoveit::SetFrame(string re_frame){
    reference_frame = re_frame;
    D.setPoseReferenceFrame(reference_frame);
    L.setPoseReferenceFrame(reference_frame);
    R.setPoseReferenceFrame(reference_frame);
}


/*  设置左/右臂相对于当前位置的位移矢量, 
    输入: 三个坐标轴方向上的相对位移量(单位m)       dy,dz参数可缺省, 默认缺省坐标方向不移动  */
void KukaMoveit::SetLdxdydz(double dx, double dy, double dz){
    Setdxdydz(L_target_pose, L_EE, dx, dy, dz);
}
void KukaMoveit::SetRdxdydz(double dx, double dy, double dz){
...
void KukaMoveit::SetLeftPose(double x, double y, double z, vector<double> o){
    SetPose(L_target_pose, x, y, z, o);
}
void KukaMoveit::SetRightPose(double x, double y, double z, vector<double> o){
    SetPose(R_target_pose, x, y, z, o);
}
/*  设置左/右臂位姿(函数重载)
    输入: cv::Point(x, y)(相机视野的像素坐标), {ox, oy, oz, ow}或{r, p, y}
        后五个参数可缺省, 默认为 MOV_Z(在 move_kuka.h 文件宏定义中修改), {0, 0, 0, 1}(沿z轴正方向)  */
void KukaMoveit::SetLeftPose(Point ptTarget, double z, vector<double> o){
...
void KukaMoveit::SetRightPose(Point ptTarget, double z, vector<double> o){
    if(o.size() != 3 && o.size() != 4)          cout << "【ERROR】Orientation value in MoveToLeftPose()\n";
    vector<double> ptTargetInFrame = PointPixel2CameraFrame(ptTarget);      // 像素坐标转换为相机坐标
...
void KukaMoveit::SetLeftJoint(double a6, double a5, double a4, double a3, double a2, double a1, double a0){
    SetJoint(L_target_joint, a6, a5, a4, a3, a2, a1, a0);
}
void KukaMoveit::SetRightJoint(double a6, double a5, double a4, double a3, double a2, double a1, double a0){
...
void KukaMoveit::ExecuteGroup(string strGroup){
...
/*  执行机械臂关节角移动
    输入: 规划组名称        参数可缺省, 默认执行双臂规划组"D"  */
void KukaMoveit::ExecuteJointGroup(string strGroup){
    moveit::planning_interface::MoveGroupInterface GROUP(strGroup);
    if(strGroup == D_GROUP){
...
void KukaMoveit::MoveLdxdydz(double dx, double dy, double dz){
    Setdxdydz(L_target_pose, L_EE, dx, dy, dz);
    ExecuteGroup(L_GROUP);
}
void KukaMoveit::MoveRdxdydz(double dx, double dy, double dz){
    Setdxdydz(R_target_pose, R_EE, dx, dy, dz);
    ExecuteGroup(R_GROUP);
}
void KukaMoveit::MoveDdxdydz(double dx, double dy, double dz){
    Setdxdydz(L_target_pose, L_EE, dx, dy, dz);
    Setdxdydz(R_target_pose, R_EE, dx, dy, dz);
...
/*  执行移动左/右臂至目标位姿
    输入: x, y, z, {ox, oy, oz, ow}或{r, p, y}      
        后五个参数可缺省, 默认为 MOV_Z(在 move_kuka.h 文件宏定义中修改), {0, 0, 0, 1}(沿z轴正方向)  */
void KukaMoveit::MoveToLeftPose(double x, double y, double z, vector<double> o){
    if(o.size() != 3 && o.size() != 4)          cout << "【ERROR】Orientation value in MoveToLeftPose()\n";
    SetPose(L_target_pose, x, y, z, o);
    ExecuteGroup(L_GROUP);
}
void KukaMoveit::MoveToRightPose(double x, double y, double z, vector<double> o){
...
/*  执行移动左/右臂至目标位姿(函数重载)
    输入: cv::Point(x, y)(相机视野的像素坐标), {ox, oy, oz, ow}或{r, p, y}
        后五个参数可缺省, 默认为 MOV_Z(在 move_kuka.h 文件宏定义中修改), {0, 0, 0, 1}(沿z轴正方向)  */
void KukaMoveit::MoveToLeftPose(Point ptTarget, double z, vector<double> o){
    if(o.size() != 3 && o.size() != 4)          cout << "【ERROR】Orientation value in MoveToLeftPose()\n";
    vector<double> ptTargetInFrame = PointPixel2CameraFrame(ptTarget);      // 像素坐标转换为相机坐标
    SetPose(L_target_pose, ptTargetInFrame[0], ptTargetInFrame[1], z, o);
    ExecuteGroup(L_GROUP);
}
void KukaMoveit::MoveToRightPose(Point ptTarget, double z, vector<double> o){
    if(o.size() != 3 && o.size() != 4)          cout << "【ERROR】Orientation value in MoveToRightPose()\n";
    vector<double> ptTargetInFrame = PointPixel2CameraFrame(ptTarget);      // 像素坐标转换为相机坐标
    SetPose(R_target_pose, ptTargetInFrame[0], ptTargetInFrame[1], z, o);
    ExecuteGroup(R_GROUP);
...
/*  执行移动左/右臂至目标关节角
    输入: a6, a5, a4, a3, a2, a1, a0(注意末端关节在第一位)      参数可缺省, 默认为(0, 0, 0, 0, 0, 0, 0)  */
void KukaMoveit::MoveToLeftJoint(double a6, double a5, double a4, double a3, double a2, double a1, double a0){
    SetJoint(L_target_joint, a6, a5, a4, a3, a2, a1, a0);
    ExecuteJointGroup(L_GROUP);
}
void KukaMoveit::MoveToRightJoint(double a6, double a5, double a4, double a3, double a2, double a1, double a0){
    SetJoint(R_target_joint, a6, a5, a4, a3, a2, a1, a0);
    ExecuteJointGroup(R_GROUP);
}
/*  执行移动左/右臂至目标关节角(函数重载)
    输入: {a0, a1, a2, a3, a4, a5, a6}, 注意末端关节在最后一位      参数可缺省, 默认为{0, 0, 0, 0, 0, 0, 0}  */
void KukaMoveit::MoveToLeftJoint(vector<double> vJoints){
...
void KukaMoveit::MoveToRightJoint(vector<double> vJoints){
...
void KukaMoveit::MoveToDualJoint(vector<double> vLeftJoints, vector<double> vRightJoints){
...
void KukaMoveit::MoveOneLeftJointToTarget(int nJointID, double dJointValue){
    if(nJointID>7 || nJointID<0)        cout << "【ERROR】nJointID in MoveOneLeftJointToTarget()\n";
    L_target_joint[nJointID] = dJointValue;
    ExecuteJointGroup(L_GROUP);
}
void KukaMoveit::MoveOneRightJointToTarget(int nJointID, double dJointValue){
...
void KukaMoveit::MoveOneLeftJointIncrease(int nJointID, double dJointIncrease){
...
void KukaMoveit::MoveOneRightJointIncrease(int nJointID, double dJointIncrease){
...
void KukaMoveit::GoHome(string strGroup){
    if(strGroup == D_GROUP || strGroup == L_GROUP)
        SetLeftJoint(3.14, 1.57);
    if(strGroup == D_GROUP || strGroup == R_GROUP)
        SetRightJoint(3.14, 1.57);
...
/*  将图像的像素坐标(col, row)转换为相机坐标的(x, y), 像素相关参数在 move_kuka.h 文件宏定义中修改
    ...
    输出: 相机坐标系下的 {x, y}  */
vector<double> KukaMoveit::PointPixel2CameraFrame(Point ptPixel){
...
/*  将RPY转换为四元数XYZW
    输入: r, p, y
    输出: {x, y, z, w}  */
vector<double> KukaMoveit::RPY2XYZW(double oz, double oy, double ox){
    vector<double> vXYZW;
